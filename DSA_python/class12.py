...
the number of tickets for each day for N days. Let A be an array of length N. Each element A[i] (where i=1 to i=N) 
denotes number of tickets to resolve on ith day. Jay is struggling to balance his work life. On some days, workload is huge 
and on other days, it is very little. Now he can procrastinate and choose to postpone up to K tickets to next day. 
However, tickets can only be postponed once. (Refer example 2 for more clarity). 
Find optimal solution where workload can be distributed as evenly as possible with above constraints and
print the maximum number of tickets he needs to resolve on given days.Constraints
1 <= T <= 50
1 <= N <= 100
1 <= K <= 100
...
rearrangement with above constraints.Test Case
 Explanation
Example 1
Input
2
...
. (In other words, 4 tickets out 5 which were postponed from day 1 has to be resolved on day 2. 
   They cannot be postponed any further). So after postponing 1 ticket array looks like [3,4,2] and maximum workload is 4, 
hence answer is 4."""

def possible(arr, k, x):
    carry = 0
